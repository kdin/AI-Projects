...
def enhancedFeatureExtractorDigit(datum):
    ...
    summation = features.totalCount()
    totalKeys = len(features.keys())
    off = totalKeys - summation
    
    ratio = float(summation)/float(totalKeys)
    ...
    for position in features:
        ...
        if position[0] == position[1] and features[position] ==1:
            diagonals += 1

        if position[0] == DIGIT_DATUM_WIDTH/2 and features[position] ==1:
            centerVertical += 1
    ...
    # features['on'] = summation
    # features['off'] = len(features.keys()) - summation
...
def enhancedPacmanFeatures(state, action):
    ...
    pacmanPosition = state.generateSuccessor(0, action).getPacmanPosition()
